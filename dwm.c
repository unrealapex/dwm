...
// TODO: check if you should define bw as an argument
...
moveresize(const Arg *arg) {
	...
	Client *c;
	c = selmon->sel;
	int x, y, w, h, nx, ny, nw, nh, bw, ox, oy, ow, oh;
...
moveresizeedge(const Arg *arg) {
	...
	c = selmon->sel;
	char e;
	int nx, ny, nw, nh, bw, ox, oy, ow, oh, bp;
	int msx, msy, dx, dy, nmx, nmy;
	int starty;
	unsigned int dui;
	Window dummy;
...
tile(Monitor *m) {
	...
	for(i = 0, my = ty = gappx, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if(i < m->nmaster) {
			...
			resize(c, m->wx + gappx, m->wy + my, mw - (2*borderpx) - gappx*(5-ns)/2, h - (2*borderpx), borderpx, 0);
			my += HEIGHT(c) + gappx;
		}
		else {
			h = (m->wh - ty) / (n - i) - gappx;
			resize(c, m->wx + mw + gappx/ns, m->wy + ty, m->ww - mw - (2*borderpx) - gappx*(5-ns)/2, h - (2*borderpx), borderpx, 0);
			ty += HEIGHT(c) + gappx;
		}
}
...
window_map(Display *dpy, Client *c, int deiconify)
{
	...
	// XMoveResizeWindow(dpy, c->win, c->x, c->y, c->w, c->bw, c->h);
	XMoveResizeWindow(dpy, c->win, c->x, c->y, c->w, c->h);
